...
contract HealthCareSystem {
    ...
    mapping(uint256 => uint256) public verify;
    ...
    mapping(uint256 => denoteNum) entitie;
    ...
    function addDoctor(
        ...
        require(
            verify[doctorId] == 0 && addresstoId[msg.sender] == 0,
    ...
    function addInsurancecompany(
        ...
        require(
            verify[_companyId] == 0 && addresstoId[msg.sender] == 0,
    ...
    function addNotCoverdMedicationInInsurance(string memory _Medication)
        public
    {
        ...
        )
    {
        ...
        return (
            InsuranceComInfo[InsuranceId].name,
            InsuranceComInfo[InsuranceId].phoneNo,
            InsuranceComInfo[InsuranceId].notCovered
        );
    }
    ...
    function addChemist(
        ...
        require(
            verify[_chemistId] == 0 && addresstoId[msg.sender] == 0,
    ...
    function addPatientInfo(
        ...
        require(
            verify[_adharCardNumber] == 0 && addresstoId[msg.sender] == 0,
            "Aadhar card or Address is already registerd/associated with another id"
        );
        require(
            InsuranceComInfo[_insuranceCompany].companyId == _insuranceCompany,
            "Insurance company is not registered"
        );
        ...
        returns (uint256[] memory)
    {
        uint256 Id = addresstoId[msg.sender];
        require(
            verify[Id] == 2 || verify[_adharCardNumber] == 1,
            "Caller should be Doctor/Patient"
        );
    ...
    function TreatPatient(
        ...
        require(
            IdtoAdress[_doctorId] == msg.sender,
            "Caller is not registered Doctor"
        );
        require(verify[_patientId] == 1, "PatientId is not registered");
        uint256 latestTreatmentid = uint256(
            keccak256(
                abi.encodePacked(block.timestamp, msg.sender, "123434231")
            )
        ) % 13421;
    ...
    function addInsuranceKeep(uint256 _patientId, string memory _medication)
        public
    {
        uint256 Id = addresstoId[msg.sender];
        require(
            verify[Id] == 1 || verify[Id] == 2,
            "Caller is not registered user"
        );
        uint256 _treatmentid = paitentInfo[_patientId].treatmentId[
            paitentInfo[_patientId].treatmentId.length - 1
        ];
        string memory medicines = treatmentInfo[_treatmentid].medicine;
        require(
            keccak256(bytes(medicines)) == keccak256(bytes(_medication)),
            "Medicines should be match for clamming insurance"
        );
        treatmentInfo[_treatmentid].InsuranceKeep.push(_medication);
    }

    function givenMedicines(uint256 _patientId)
        public
        view
